...
# Optional badge (may be cached by some CDNs)
VIEWS_BADGE = (
    '<img src="https://hits.seeyoufarm.com/api/count/incr/'
    'badge.svg?url=https%3A%2F%2Fjoeshmojo.github.io%2FWatershedForecast%2F'
    '&count_bg=%2379C83D&title_bg=%23555555&icon=&icon_color=%23E7E7E7'
    '&title=views&edge_flat=false" alt="views badge" />'
)

# Inline JS counter that always shows a number
# Namespace/key pair can be anything stable; using your site as namespace & path as key.
COUNTAPI_SNIPPET = r"""
  <script>
    (async () => {
      try {
        const res = await fetch("https://api.countapi.xyz/hit/joeshmojo.github.io/WatershedForecast", {cache:"no-store"});
...
"""
  ...
  <footer>
    ...
    <p class="views">
      <strong>Views:</strong> <span id="view-count">—</span>
      <br/>{VIEWS_BADGE}
    </p>
  </footer>
{COUNTAPI_SNIPPET}
